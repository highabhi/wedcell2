...
const EditListedItems = () => {
  ...
  const addHandler = async () => {
    ...
    mainform.append('image', {
      name: mainImage.name,
      type: mainImage.type,
      uri: URL.createObjectURL(mainImage)
    })
      ...
      if (galleryImages.length) {
        ...
        Object.keys(galleryImages).forEach(function (key) {
          ...
          gallery.append(`images`, {
            name: galleryImages[key].name,
            type: galleryImages[key].type,
            uri: URL.createObjectURL(galleryImages[key])
          })
        });
    ...
    if (brochure) {
      ...
      brocherdata.append(`brochure`, {
        name: brochure.name,
        type: brochure.type,
        uri: URL.createObjectURL(brochure)
      })
  ...
  const uploadAlbums = async (id, item, config) => {
    ...
    Object.keys(item.value).forEach(function (key) {
      ...
      data.append(`albums`, {
        ...
        type: item.value[key].type,
        uri: URL.createObjectURL(item.value[key])
      })
    });
    await axios.post(
            ...
            {allowedVendors.map((data, key) => (
              <div className="row mt-3 mb-3" key={key}>
                ...
                <div className="col-md-4">
                  ...
                  }} placeholder='0000000000' checked={data.value} ></input>
                </div>
              </div>
            ))}
        ...
        <div className="row">
          ...
          <div className="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6">
            <div className={Styles.name_block}>
              ...
              <br></br>
              <label className={Styles.label}>
                <input onChange={brochureHandler} type="file" id="myfile" name="myfile" />
              </label>
            </div>
          </div>
            ...
            <div className={Styles.name_block}>
              ...
              {albums.map((album, key) => (
                <label key={key} className={Styles.label}>
                  <input type="text" onChange={onChangeAlbumHandler(key)} placeholder='0000000000' className={Styles.phone_tag}></input>
